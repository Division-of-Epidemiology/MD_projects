...
# ============================================================
#  Overdose Surveillance Dashboard — Combined (ED / EMS / ME)
#  Toggle between datasets via dropdown
#  Thresholds: 1 SD = Elevated, 2 SD = Anomaly Detected
#  Geospatial: Census Tract choropleth (EMS/ME exact, ED estimated)
# ============================================================
...
# ── 1. Load Daily Case Data (for KPIs / trend / calendar) ───

load_daily <- function(path, date_col, label) {
...
dataset_list <- list(
  "ED — Emergency Department"        = ed_daily,
  "EMS — Emergency Medical Services" = ems_daily,
  "ME — Medical Examiner"            = me_daily
)

# ── 2. Load Tract-Level Data (from step2_tract_join.R outputs) ──

# EMS/ME — exact point-in-polygon joins, one row per case
ems_tract_raw <- read.csv(
  "C:/Users/mdickson/OneDrive - Metro Nashville Gov/Desktop/CVS_Files/EMS_tract.csv",
  stringsAsFactors = FALSE
) %>%
  mutate(GEOID = as.character(GEOID),
         date  = as.Date(Incident_Date, format = "%m/%d/%Y")) %>%
  filter(!is.na(GEOID), !is.na(date)) %>%
  mutate(year = lubridate::year(date))

me_tract_raw <- read.csv(
  "C:/Users/mdickson/OneDrive - Metro Nashville Gov/Desktop/CVS_Files/ME_tract.csv",
  stringsAsFactors = FALSE
) %>%
  mutate(GEOID = as.character(GEOID),
         date  = as.Date(Exam_Date, format = "%m/%d/%Y")) %>%
  filter(!is.na(GEOID), !is.na(date)) %>%
  mutate(year = lubridate::year(date))

# ED — apportioned estimates (already daily x tract x est_cases)
ed_tract_raw <- read.csv(
...
tract_list <- list(
  "ED — Emergency Department"        = ed_tract_raw,
  "EMS — Emergency Medical Services" = ems_tract_raw,
  "ME — Medical Examiner"            = me_tract_raw
)

# Flag which datasets are estimated vs exact (for UI disclaimer)
tract_is_estimated <- list(
  "ED — Emergency Department"        = TRUE,
  "EMS — Emergency Medical Services" = FALSE,
  "ME — Medical Examiner"            = FALSE
)

# ── 3. Load Census Tract Shapefile ───────────────────────────
tract_shapes <- st_read(
  "C:/Users/mdickson/OneDrive - Metro Nashville Gov/Desktop/CVS_Files/CT_boundaries/ct.shp",
  quiet = TRUE
) %>%
  st_transform(crs = 4326) %>%
  select(GEOID = GEOCODE, geometry) %>%
  mutate(GEOID = as.character(GEOID))

# ── 4. Classify Days (SD-based) ──────────────────────────────
...
ui <- fluidPage(
  ...
  div(class = "panel-grid", style = "padding-bottom:36px;",
      div(class = "panel",
          div(class = "panel-header",
              tags$h2("🗺️ Cases by Census Tract"),
              uiOutput("map_badge")
          ),
          div(class = "panel-body", leafletOutput("tract_map", height = "440px"))
      ),
      div(class = "panel",
          div(class = "panel-header",
              tags$h2("📍 Top 10 Census Tracts"),
              div(class = "panel-badge", "by total cases")
          ),
          div(style = "overflow-x:auto;", tableOutput("tract_table"))
      )
  )
)
...
server <- function(input, output, session) {
  ...
  # Tract-level aggregation: EMS/ME use 'cases' (count of rows),
  # ED uses 'est_cases' (apportioned, can be fractional)
  tract_data <- reactive({
    req(input$sel_year)
    raw <- tract_list[[input$sel_dataset]] %>%
      filter(year == as.integer(input$sel_year))
    
    if (input$sel_dataset == "ED — Emergency Department") {
      raw %>%
        group_by(GEOID) %>%
        summarise(cases = sum(est_cases), .groups = "drop") %>%
        arrange(desc(cases))
    } else {
      raw %>%
        group_by(GEOID) %>%
  ...
  is_estimated <- reactive({
    tract_is_estimated[[input$sel_dataset]]
  })
  
  output$map_badge <- renderUI({
    if (is_estimated()) {
      div(class = "panel-badge estimate", "estimated (zip apportioned)")
    } else {
      div(class = "panel-badge", "exact (geocoded)")
    }
  })
  ...
  output$data_quality_note <- renderUI({
    if (is_estimated()) {
      div(class = "data-quality-note",
          "⚠ ED data only includes zip code, not address. Tract-level counts below are ",
          tags$b("estimated"), " by apportioning each zip's daily cases across overlapping tracts ",
          "using HUD's residential ZIP-to-Tract crosswalk. Treat these as approximations, not exact counts."
      )
    } else {
      NULL
    }
  ...
  # ── Census tract choropleth map
  output$tract_map <- renderLeaflet({
    td <- tract_data()
    
    map_df <- tract_shapes %>%
      left_join(td, by = "GEOID") %>%
      mutate(cases = ifelse(is.na(cases), 0, cases))
    
    pal <- colorNumeric(
      palette = c("#6cb4cf", "#e8b339", "#e8743b", "#c62828", "#7a0c0c"),
      domain  = map_df$cases
    )
    
    label_fmt <- if (is_estimated()) {
      sprintf("Tract %s: %.1f est. cases", map_df$GEOID, map_df$cases)
    } else {
      sprintf("Tract %s: %d cases", map_df$GEOID, as.integer(map_df$cases))
    }
    
    popup_fmt <- if (is_estimated()) {
      sprintf("<b>Tract: %s</b><br>Estimated Cases: <b>%.1f</b><br><i>Zip-apportioned estimate</i>",
              map_df$GEOID, map_df$cases)
    } else {
      sprintf("<b>Tract: %s</b><br>Cases: <b>%d</b>", map_df$GEOID, as.integer(map_df$cases))
    }
    ...
    leaflet(map_df) %>%
      ...
      addPolygons(
        ...
        highlightOptions = highlightOptions(
          weight = 3, color = "#1d6ae5", fillOpacity = 0.9, bringToFront = TRUE
        )
      ) %>%
      addLegend(
        ...
        pal      = pal,
        values   = ~cases,
        title    = if (is_estimated()) "Est. Cases" else "Cases",
        opacity  = 0.8
      )
  })
  
  # ── Top 10 census tracts table
  output$tract_table <- renderTable({
    td <- tract_data() %>% slice_head(n = 10)
    total <- sum(tract_data()$cases)
    ...
    if (is_estimated()) {
      td %>%
        mutate(
          Rank   = row_number(),
          Tract  = GEOID,
          `Est. Cases` = round(cases, 1),
          `% of Total` = paste0(round(100 * cases / total, 1), "%")
        ) %>%
        select(Rank, Tract, `Est. Cases`, `% of Total`)
    } else {
      td %>%
        mutate(
          Rank   = row_number(),
        ...
        select(Rank, Tract, Cases, `% of Total`)
    }
  }, striped = FALSE, hover = TRUE, bordered = FALSE, spacing = "s", align = "l", rownames = FALSE)
}
