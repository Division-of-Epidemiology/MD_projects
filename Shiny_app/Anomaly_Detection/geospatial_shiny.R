...
# Zip code centroids for Davidson County (lat/lng)
zip_centroids <- data.frame(
  zip = c("37013","37027","37072","37076","37080","37115","37138",
          "37189","37201","37203","37204","37205","37206","37207",
          "37208","37209","37210","37211","37212","37213","37214",
          "37215","37216","37217","37218","37219","37220","37221"),
  lat = c(36.0614,36.0171,36.2484,36.1871,36.2601,36.2440,36.1715,
          36.2834,36.1659,36.1517,36.1315,36.1093,36.1862,36.2185,
          36.1762,36.1548,36.1221,36.0867,36.1466,36.1659,36.1715,
          36.1093,36.2185,36.1659,36.0614,36.1659,36.0614,36.1274),
  lng = c(-86.6727,-86.8258,-86.7050,-86.6282,-86.9022,-86.6990,-86.6838,
          -86.7748,-86.7789,-86.7844,-86.7789,-86.8258,-86.7282,-86.7534,
          -86.8090,-86.8344,-86.7844,-86.7282,-86.8090,-86.7789,-86.6838,
          -86.8512,-86.7282,-86.7789,-86.6727,-86.7789,-86.6727,-86.9022),
  ...
  tags$head(
    ...
    tags$style(HTML("
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      body { background:#f4f6f8; color:#1a1a2e; font-family:'Roboto',sans-serif; font-size:15px; min-height:100vh; }
      .dash-header { background:#ffffff; border-bottom:2px solid #e0e4ea; padding:24px 36px 20px; display:flex; align-items:flex-end; gap:18px; }
      .dash-header h1 { font-family:'Merriweather',serif; font-size:1.75rem; font-weight:900; color:#000000; line-height:1; }
      .dash-header .subtitle { font-size:0.78rem; color:#555e6d; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:4px; font-weight:600; }
      .dataset-badge { display:inline-block; background:#1d6ae5; color:#ffffff; border-radius:6px; padding:5px 14px; font-size:0.82rem; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; align-self:flex-end; margin-left:12px; margin-bottom:3px; }
      .controls-bar { background:#ffffff; border-bottom:1px solid #e0e4ea; padding:14px 36px; display:flex; align-items:center; gap:28px; flex-wrap:wrap; }
      .controls-bar label { font-size:0.78rem; letter-spacing:0; text-transform:uppercase; color:#333d4a; margin-bottom:4px; display:block; font-weight:700; }
      .controls-bar .form-control, .controls-bar .selectize-input { background:#f4f6f8 !important; border:1px solid #c8d0da !important; color:#1a1a2e !important; border-radius:6px !important; font-family:'Roboto',sans-serif !important; font-size:0.85rem !important; padding:6px 10px !important; }
      .shiny-input-container { margin-bottom:0 !important; }
      .legend-row { display:flex; align-items:center; gap:20px; margin-left:auto; }
      .legend-item { display:flex; align-items:center; gap:7px; font-size:0.78rem; color:#333d4a; font-weight:600; }
      .legend-dot { width:12px; height:12px; border-radius:50%; flex-shrink:0; }
      .kpi-row { display:flex; gap:16px; padding:20px 36px; flex-wrap:wrap; }
      .kpi-card { background:#ffffff; border:1px solid #e0e4ea; border-radius:10px; padding:16px 22px; flex:1; min-width:140px; box-shadow:0 1px 4px rgba(0,0,0,0.06); }
      .kpi-card .kpi-label { font-size:0.72rem; letter-spacing:0.08em; text-transform:uppercase; color:#555e6d; margin-bottom:6px; font-weight:700; }
      .kpi-card .kpi-value { font-family:'Merriweather',serif; font-size:1.9rem; font-weight:900; line-height:1; color:#000000; }
      .kpi-card .kpi-sub { font-size:0.72rem; color:#555e6d; margin-top:4px; font-weight:500; }
      ...
      .panel-grid { display:grid; grid-template-columns:1fr 1fr; gap:20px; padding:0 36px 20px; }
      @media (max-width:900px) { .panel-grid { grid-template-columns:1fr; } }
      .panel { background:#ffffff; border:1px solid #e0e4ea; border-radius:10px; overflow:hidden; box-shadow:0 1px 4px rgba(0,0,0,0.06); }
      .panel-full { background:#ffffff; border:1px solid #e0e4ea; border-radius:10px; overflow:hidden; margin:0 36px 20px; box-shadow:0 1px 4px rgba(0,0,0,0.06); }
      .panel-header { padding:14px 20px 12px; border-bottom:1px solid #e0e4ea; display:flex; align-items:center; gap:10px; background:#f8f9fb; }
      .panel-header h2 { font-family:'Merriweather',serif; font-size:1.3rem; font-weight:700; color:#000000; }
      .panel-header .panel-badge { font-size:0.72rem; background:#e8edf3; border:1px solid #c8d0da; color:#444e5c; border-radius:4px; padding:4px 10px; letter-spacing:0.06em; text-transform:uppercase; margin-left:auto; font-weight:600; }
      .panel-body { padding:8px 4px; }
      .shiny-html-output table { width:100%; border-collapse:collapse; font-size:0.78rem; }
      .shiny-html-output table th { padding:7px 14px; text-align:left; font-size:0.72rem; letter-spacing:0.06em; text-transform:uppercase; color:#333d4a; border-bottom:2px solid #e0e4ea; font-weight:700; background:#f8f9fb; }
      .shiny-html-output table td { padding:6px 14px; border-bottom:1px solid #f0f2f5; color:#1a1a2e; font-weight:500; }
  ...
  output$zip_map <- renderLeaflet({
    ...
    if (nrow(map_df) == 0) {
      leaflet() %>%
        ...
        addControl("<b>No zip code data available</b>", position = "topright")
    } else {
      # Color palette based on case counts
      pal <- colorNumeric(
        palette = c("#2A9D8F", "#F4A261", "#E63946"),
        domain  = map_df$cases
      )
      
      # Radius scaled to cases
      map_df$radius <- scales::rescale(map_df$cases, to = c(8, 35))
